...
public abstract class AbstractImageLoader<T> implements ImageLoader {
    ...
    public AbstractImageLoader(ImageHolder holder, RichTextConfig config, TextView textView, DrawableWrapper drawableWrapper, ImageLoadNotify iln, SourceDecode<T> sourceDecode) {
        this.holder = holder;
        this.config = config;
    ...
    public void onLoading() {
        ...
        if (placeHolder.getBounds().right == 0) {
            int width = textViewWeakReference.get().getWidth();
            placeHolder.setBounds(0, 0, width, width / 2);
        }
    ...
    public void onFailure(Exception e) {
        ...
        if (errorImage.getBounds().right == 0) {
            int width = textViewWeakReference.get().getWidth();
            errorImage.setBounds(0, 0, width, width / 2);
        }
