...
public class XLTaskHelper {
     ...
     * @throws Exception
     */
    public synchronized long addMagentTask(String url, final String savePath, String fileName) throws Exception {
        if (url.startsWith("thunder://")) {
            url = mXlDownloadManager.parserThunderUrl(url);
        }
        if (url.startsWith("ftp://") || url.startsWith("ed2k://")) {
            return addThunderTask(url, savePath, fileName);
        }
        if (url.startsWith("magnet:?")) {
            ...
            throw new Exception("地址无法识别：" + url);
        }
    }
